/*********************************************************
	     	     Semana 6 - Programa 4
	Demo - Exportando resultados de uma procedure
	
**********************************************************
	Através da utilização do ODS (SAS Output Delivery System),
é possível exportar resultados de uma procedure (PROC PRINT,
por exemplo) em um arquivo mapeado pelo comando ODS

	Sintaxe geral:
	
	ODS <destination> <destination_specifications>
		* código SAS para produzir o output
	ODS <destionation> CLOSE;
	
	Sintaxe para gerar outputs em arquivos EXCEL:
...
*1) Exemplo 1: gerando arquivo CSV a partir do PROC PRINT;
%let outpath=~/EPG194/output;
ods csvall file="&outpath/cars2.csv";
...
* Visualizar estilos disponíveis no SAS;
proc template;
	list styles;
run;
